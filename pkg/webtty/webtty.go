...
func (wt *WebTTY) Run(ctx context.Context) error {
	...
	if err != nil {
		return fmt.Errorf("failed to send initializing message, %v", err)
	}
	...
	for {
		...
		case err = <-errs:
			return err
		case <-ticker.C:
			err = wt.sendPingMessage()
		}
	}
}

func (wt *WebTTY) sendPingMessage() error {
	wt.writeMutex.Lock()
	defer wt.writeMutex.Unlock()
	return wt.masterConn.Ping()
}

func (wt *WebTTY) sendInitializeMessage() error {
	err := wt.masterWrite(append([]byte{SetWindowTitle}, wt.windowTitle...))
	if err != nil {
		return fmt.Errorf("failed to send window title, %v", err)
	}
	...
	if wt.reconnect > 0 {
		...
		if err != nil {
			return fmt.Errorf("failed to set reconnect, %v", err)
		}
	}
...
func (wt *WebTTY) masterWrite(data []byte) error {
	...
	if err != nil {
		return fmt.Errorf("failed to write to master, %v", err)
	}
...
func (wt *WebTTY) handleMasterReadEvent(data []byte) error {
	if len(data) == 0 {
		return fmt.Errorf("unexpected zero length read from master")
	}
	...
	case Input:
		...
		if err != nil {
			return fmt.Errorf("failed to write received data to slave, %v", err)
		}

	case Ping:
		err := wt.masterWrite([]byte{Pong})
		if err != nil {
			return fmt.Errorf("failed to return Pong message to master, %v", err)
		}
	...
	case ResizeTerminal:
		...
		if len(data) <= 1 {
			return fmt.Errorf("received malformed remote command for terminal resize: empty payload")
		}

		var args argResizeTerminal
		err := json.Unmarshal(data[1:], &args)
		if err != nil {
			return fmt.Errorf("received malformed data for terminal resize, %v", err)
		}
		rows := wt.rows
		if rows == 0 {
			rows = int(args.Rows)
